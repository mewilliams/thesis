...
\end{eqnarray}
equation~\ref{eq:S} is linearized \emph{um?} and a linear fit ((y = ax + b).. a = $\frac{1}{\sqrt{4K_xt}}$ and b = $\frac{-x_c}{\sqrt{4K_xt}}$) is applied. To solve for $K_x$, a value of \emph{t} must be determined, and in this case it is appropriate to use the time between when the front floods past the sensors and subsequent ebb of said front. This method assumes an infitely sharp front initially passes by the sensors, and will overestimate \emph{K$_x$}, thus an estimation on the initial time from a sharp front to the flood front is made. By setting a step function at the mouth, and approximating advection velocities U = 0.1 m s$^{-1}$ and a lenght from the ocean to NM of 100 m, initial time, t$_o$ is 1000 s. This time is then added to the time between a selected isopycnal passing by the sensor. Specific considerations in selecting the isopycnal to follow, and implications will be discussed \emph{later (in sec. X)}. 
...
\emph{Actually, this is looking at dispersion due to vertical shear.. i think should be $K_z$... obviously much smaller...}
\begin{eqnarray}
